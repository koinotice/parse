...
class HashTask {
    ...
    async test() {
        ...
        if (order.receipt) {
            ...
            if (order.receipt.status == true) {
                that.test()
                logger.info("HashTask success blockNumber %s roomId %s", order.receipt.blockNumber + 2, 1)
            } else {
                //如果操作失败，2个区块后重试

                logger.error("HashTask fail blockNumber %s roomId %s", order.receipt.blockNumber + 2, 1)
            }
        }
